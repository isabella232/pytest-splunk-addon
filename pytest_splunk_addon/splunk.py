...
def uf_kubernetes(request):
    ...
    if ( "PYTEST_XDIST_WORKER" not in os.environ or os.environ.get("PYTEST_XDIST_WORKER") == "gw0"):
        ...
        update_splunk_sc4s_uf_deployments(folder="k8s_manifests/uf",file="uf_deployment")
        LOGGER.info("Setting up UF")
        uf_setup = subprocess.run('sh k8s_manifests/uf/uf_setup.sh',capture_output=True,shell=True)
...
def update_splunk_sc4s_uf_deployments(folder,file):
    try:
...
def splunk_kubernetes(request):
    ...
    if ( "PYTEST_XDIST_WORKER" not in os.environ or os.environ.get("PYTEST_XDIST_WORKER") == "gw0"):
        ...
        update_splunk_sc4s_uf_deployments(folder="k8s_manifests/splunk_standalone",file="splunk_standalone")
...
def sc4s_kubernetes():
    ...
    if ( "PYTEST_XDIST_WORKER" not in os.environ or os.environ.get("PYTEST_XDIST_WORKER") == "gw0"):
        ...
        update_splunk_sc4s_uf_deployments(folder="k8s_manifests/sc4s",file="sc4s_deployment")
